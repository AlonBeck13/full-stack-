...
const App = () => {
  ...
  const addNote = (event) => {
    ...
    noteService.create(noteObject).then((initialNotes) => {
      ...
      setNewNote('')
      setErrorMessage(
        `Added '${nt}' was already removed from server`
      )
      setTimeout(() => {
  ...
  const handleImportanceChange = (id) => {
    ...
    noteService.update(id, changedNote).then(initialNotes => {
      setNotes(notes.map(n => n.id === id ? initialNotes : n))
    }).catch(error => {
      setErrorMessage(
    ...
    <div>
      ...
      <ul>
        {notesToShow.map((note) => (
          <Note key={note.id} note={note} helpFunction={() => handleImportanceChange(note.id)} />
        ))}
      </ul>
